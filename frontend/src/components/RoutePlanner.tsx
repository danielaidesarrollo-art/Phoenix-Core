...
const RoutePlanner: React.FC = () => {
    ...
    const getAntibioticSchedule = (patient: Patient, dateStr: string): string[] => {
        if (!patient.antibiotico || !patient.terapias || !patient.terapias['Aplicación de terapia antibiótica']) return [];
    ...
    const calculatedRoute = useMemo(() => {
        ...
        const filtered = patients.filter(p => {
            ...
            const patientNeeds = p.terapias ? Object.entries(p.terapias)
            ...
            const hasAntibioticNeed = isAuxiliar && p.terapias && p.terapias['Aplicación de terapia antibiótica'];
    ...
    useEffect(() => {
        ...
        try {
            ...
            const coveragePolygon = new window.google.maps.Polygon({
                paths: COVERAGE_POLYGON.map(p => ({ lat: p.lat, lng: p.lng })),
                strokeColor: "#0D47A1",
                strokeOpacity: 0.8,
                strokeWeight: 2,
                fillColor: "#0D47A1",
