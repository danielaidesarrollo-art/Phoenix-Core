...
import { PhoenixAPI } from '../../utils/api_client';
import { CameraAdapter } from '../../utils/cameraAdapter';
                                    ...
                                    {analysisResult?.specialized_dressing && (
                                        <div className="mt-8 pt-8 border-t border-white/5">
                                            <div className="flex items-center gap-3 mb-4">
                                                <span className="material-symbols-outlined text-green-400">inventory_2</span>
                                                <h4 className="text-sm font-bold text-white uppercase tracking-widest">Recommended Specialized Products</h4>
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                                                    <p className="text-[10px] text-blue-300 font-mono uppercase mb-1">Dressing / Technology</p>
                                                    <p className="text-white text-sm font-bold">{analysisResult.specialized_dressing}</p>
                                                </div>
                                                <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                                                    <p className="text-[10px] text-green-300 font-mono uppercase mb-1">Laboratory Provider</p>
                                                    <p className="text-white text-sm font-bold">
                                                        {analysisResult.specialized_dressing.includes('Smith') ? 'Smith & Nephew' : 'Mölnlycke'}
                                                    </p>
                                                </div>
                                            </div>
                                            {analysisResult.specialized_dressing.includes('Versajet') && (
                                                <div className="mt-4 p-4 rounded-xl bg-blue-500/10 border border-blue-500/30">
                                                    <p className="text-blue-300 text-[10px] font-bold uppercase mb-1">Hydrosurgery Advisory</p>
                                                    <p className="text-white text-xs">Versajet usage recommended for precision debridement of non-viable tissue while preserving healthy underlying structure.</p>
                                                </div>
                                            )}
                                        </div>
                                    )}
