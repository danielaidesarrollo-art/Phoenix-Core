        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {terapias['Aplicación de terapia antibiótica'] && (
                <Card title="Detalles de Terapia Antibiótica" className="mt-4 bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Select label="Medicamento" options={ANTIBIOTICOS} value={antibiotico.medicamento} onChange={e => setAntibiotico((p: Partial<AntibioticTreatment>) => ({ ...p, medicamento: e.target.value }))} required />
                        <div>
                            <Input label="Dosis (mg)" type="number" value={antibiotico.miligramos || ''} onChange={e => setAntibiotico((p: Partial<AntibioticTreatment>) => ({ ...p, miligramos: parseFloat(e.target.value) }))} required />
                        ...
                        <div>
                            <Input label="Frecuencia (horas)" type="number" value={antibiotico.frecuenciaHoras || ''} onChange={e => setAntibiotico((p: Partial<AntibioticTreatment>) => ({ ...p, frecuenciaHoras: parseInt(e.target.value) }))} required />
                        ...
                        <div className="md:col-span-2 grid grid-cols-2 gap-4">
                            <Input label="Fecha de Inicio" type="date" value={antibiotico.fechaInicio || ''} onChange={e => setAntibiotico((p: Partial<AntibioticTreatment>) => ({ ...p, fechaInicio: e.target.value }))} required />
                            <Input label="Fecha de Terminación" type="date" value={antibiotico.fechaTerminacion || ''} onChange={e => setAntibiotico((p: Partial<AntibioticTreatment>) => ({ ...p, fechaTerminacion: e.target.value }))} required />
                        </div>
                    </div>
                </Card>
            )}
            ...
            {terapias['Manejo de Sondas'] && (
                <Card title="Detalles de Sondas" className="mt-4 bg-gray-50">
                    <Select label="Tipo de Sonda" options={SONDA_TIPOS} value={sondaInfo.tipo} onChange={e => setSondaInfo((p: any) => ({ ...p, tipo: e.target.value }))} required />
                </Card>
            )}
            ...
            {terapias['curación mayor en casa por enfermería'] && (
                <Card title="Detalles de Heridas/Curaciones" className="mt-4 bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input label="Tipo de Herida" type="text" value={heridaInfo.tipo} onChange={e => setHeridaInfo((p: any) => ({ ...p, tipo: e.target.value }))} required />
                        <Input label="Localización" type="text" value={heridaInfo.localizacion} onChange={e => setHeridaInfo((p: any) => ({ ...p, localizacion: e.target.value }))} required />
                    </div>
                </Card>
            )}
            ...
            {terapias['Toma de glucometrías'] && (
                <Card title="Detalles de Glucometrías" className="mt-4 bg-gray-50">
                    <Select label="Frecuencia" options={GLUCOMETRIA_FRECUENCIAS} value={glucometriaInfo.frecuencia} onChange={e => setGlucometriaInfo((p: any) => ({ ...p, frecuencia: e.target.value }))} required />
                </Card>
            )}
