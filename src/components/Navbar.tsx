...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, activeView, isInstallable, install }) => {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center gap-2 md:gap-4 overflow-x-auto">
                        ...
                        <NavItem icon={Icons.Healing} label="Clínica Heridas" isActive={activeView === 'woundclinic'} onClick={() => onNavigate('woundclinic')} />
                        ...
                        {user?.cargo === 'JEFE MEDICO' && (
                            <NavItem icon={Icons.Admin} label="Admin" isActive={activeView === 'admin'} onClick={() => onNavigate('admin')} />
                        )}
                    </div>
                    <div className="flex items-center gap-4">
                        {isInstallable && (
                            <button
                                onClick={install}
                                className="hidden md:flex items-center gap-2 px-3 py-1 bg-green-500 text-white rounded-full text-[10px] font-bold animate-bounce hover:bg-green-600 transition-all shadow-lg"
                            >
